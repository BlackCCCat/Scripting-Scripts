  ...
  return (
    <>
      <Rectangle
        ...
        frame={{
          width: box.width + hitPad * 2,
          height: box.height + hitPad * 2,
        }}
...
export default function App({
  ...
  const handleUIImage = useCallback(async (img: UIImage | null) => {
    ...
    try {
      ...
      const recognized = (result.candidates as any[]).map(
        (c: any, i: number) => ({
          id: i.toString(),
          content: c.content as string,
          confidence: c.confidence as number,
          boundingBox: c.boundingBox as {
            x: number;
            y: number;
            width: number;
            height: number;
  ...
  const pickPhoto = useCallback(async () => {
    try {
      const results = await (Photos as any).pick({
        limit: 1,
        filter: (PHPickerFilter as any).images(),
      });
  ...
  const pickFile = useCallback(async () => {
    try {
      const paths = await (DocumentPicker as any).pickFiles({
        types: ["public.image"],
  ...
  const resetItem = useCallback((id: string) => {
    setItems((prev) =>
      prev.map((it) => (it.id === id ? { ...it, edited: undefined } : it)),
    );
  }, []);
  ...
  const mappedBoxes = useMemo(() => {
    ...
    return items.map((it) => ({
      id: it.id,
      left: it.boundingBox.x,
      top: it.boundingBox.y,
  ...
  useEffect(() => {
    setEditorValue(
      selectedItem ? (selectedItem.edited ?? selectedItem.content) : "",
    );
  }, [selectedItem]);
  ...
  const selectItem = useCallback(
    (id: string) => {
      ...
      if (selectedId) {
        ...
        setItems((prev) =>
          prev.map((it) =>
            it.id === selectedId ? { ...it, edited: editorValue } : it,
          ),
        );
      }
      setSelectedId(id);
  ...
  function boxToDisplay(
    ...
    const top =
      (isNormalized
        ? (1 - box.y - box.height) * imgH
        : imgH - (box.y + box.height)) *
        scale +
      offsetY;
  ...
  const multiSelectText = useMemo(() => {
    ...
    for (const item of sorted) {
      ...
      if (!last || Math.abs(item.centerY - last.centerY) > lineThreshold) {
        ...
        last.centerY =
          (last.centerY * (last.items.length - 1) + item.centerY) /
          last.items.length;
      }
    }
      ...
      <ZStack>
        <VStack
          ...
          toast={{
            ...
            onChanged: (v: boolean) => setToastShown(v),
            duration: 2,
            position: "bottom",
          }}
        >
          ...
          {image ? (
            <GroupBox
              label={<Label title="操作" systemImage="slider.horizontal.3" />}
            >
              <HStack spacing={8}>
                <ToolbarButton
                  title="重置图片"
                ...
                <ToolbarButton
                  title="多选"
                  systemImage={
                    multiSelect ? "checkmark.circle.fill" : "checkmark.circle"
                  }
              ...
              <GeometryReader>
                {({ size }: any) => {
                  ...
                  const effectiveOffset = {
                    x: offset.x + gestureOffset.x,
                    y: offset.y + gestureOffset.y,
                  };
                  ...
                  const maxPanX = Math.max(
                    0,
                    (effectiveDispW - size.width) / 2,
                  );
                  const maxPanY = Math.max(
                    0,
                    (effectiveDispH - size.height) / 2,
                  );

                  const dragG = DragGesture({
                    minDistance: 10,
                      ...
                      <ZStack
                        ...
                        gesture={MagnifyGesture()
                          .onChanged((v) => {
                            ...
                            const offsetLiveX =
                              offset.x + ratio * (p.x - center.x - offset.x);
                            const offsetLiveY =
                              offset.y + ratio * (p.y - center.y - offset.y);
                            // clamp live offset
                            const liveMaxPanX = Math.max(
                              0,
                              (transforms.dispW * sLive - size.width) / 2,
                            );
                            const liveMaxPanY = Math.max(
                              0,
                              (transforms.dispH * sLive - size.height) / 2,
                            );
                            const clampedLiveX = clamp(
                              offsetLiveX,
                              -liveMaxPanX,
                              liveMaxPanX,
                            );
                            const clampedLiveY = clamp(
                              offsetLiveY,
                              -liveMaxPanY,
                              liveMaxPanY,
                            );
                            setGestureOffset({
                              x: clampedLiveX - offset.x,
                              y: clampedLiveY - offset.y,
                            });
                            setToastShown(false);
                          })
                          .onEnded((v) => {
                            ...
                            const newOffsetY =
                              offset.y + ratio * (p.y - center.y - offset.y);
                            const newMaxPanX = Math.max(
                              0,
                              (transforms.dispW * sNew - size.width) / 2,
                            );
                            const newMaxPanY = Math.max(
                              0,
                              (transforms.dispH * sNew - size.height) / 2,
                            );
                        ...
                        highPriorityGesture={TapGesture(2).onEnded(() => {
                          const p = touchPoint ?? {
                            x: size.width / 2,
                            y: size.height / 2,
                          };
                      ...
                      >
                        {displayImage ? (
                          <Image
                            ...
                            resizable={true}
                            frame={{
                              width: transforms.dispW,
                              height: transforms.dispH,
                            }}
                            position={{
                              x: transforms.offsetX + transforms.dispW / 2,
                              y: transforms.offsetY + transforms.dispH / 2,
                            }}
                            gesture={DragGesture({
                              minDistance: 0,
                              coordinateSpace: "local",
          ...
          {image && multiSelect && selectedIds.length > 0 ? (
            <GroupBox
              label={<Label title="多选结果" systemImage="checkmark.circle" />}
            >
          ...
          {selectedItem ? (
            <GroupBox
              label={
                <Label
                  title={`#${parseInt(selectedItem.id) + 1}`}
                  systemImage="pencil"
                />
              }
            >
                ...
                <HStack spacing={8}>
                  ...
                  <Button
                    ...
                    action={() => {
                      ...
                      setEditorValue(selectedItem.content);
                    }}
                  />
