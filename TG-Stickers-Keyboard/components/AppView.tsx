    ...
    <NavigationStack>
      <VStack
        ...
        toolbar={{
          ...
          topBarTrailing: (
            <HStack spacing={8}>
              <Button title="" systemImage="plus" disabled={busy} action={() => setShowImporter(true)} />
...
function ImportSheet(props: {
  ...
  async function loadPreview() {
    ...
    try {
      const nextPreview = await fetchStickerSetPreview(props.botToken, link, (next) => {
        setProgress(next)
        setStatus(next.message)
      })
      ...
      <VStack
        ...
        toolbar={{
          ...
          topBarTrailing: preview
            ...
            : <Button title="读取" disabled={busy || !link.trim()} action={() => void loadPreview()} />,
        }}
      >
...
function PackCard(props: { pack: StickerPack; onManage: () => void; onDelete: () => void }) {
  const unsupported = props.pack.stickers.filter((item) => item.kind !== "static").length
    ...
    <VStack spacing={12} padding={{ top: 14, bottom: 14, leading: 14, trailing: 14 }} glassEffect={CARD}>
      <HStack spacing={10} frame={{ maxWidth: "infinity" }}>
        <VStack spacing={3} frame={{ maxWidth: "infinity", alignment: "leading" as any }}>
          ...
          <Text font="caption" foregroundStyle="secondaryLabel" frame={{ maxWidth: "infinity", alignment: "leading" as any }}>
            {props.pack.stickers.length} 个贴纸{unsupported ? `，${unsupported} 个动态/视频仅缓存` : ""}
          </Text>
        </VStack>
