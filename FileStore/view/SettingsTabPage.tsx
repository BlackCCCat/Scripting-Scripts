...
import { renameWithPrompt } from "../manager/utils";
...
interface DirectoryBookmarkOption extends Bookmark {
  mounted: boolean;
}
...
function MountedDirectoriesSettingsPage({ onBookmarksChange }: { onBookmarksChange?: () => void }) {
  const [directoryBookmarks, setDirectoryBookmarks] = useState<DirectoryBookmarkOption[]>(getDirectoryBookmarkOptions);

  const refresh = () => {
    setDirectoryBookmarks(getDirectoryBookmarkOptions());
    onBookmarksChange?.();
  };

  const handleRename = async (bookmark: DirectoryBookmarkOption) => {
    const newName = await renameWithPrompt(bookmark.name);
    if (!newName) return;
    if (directoryBookmarks.some((item) => item.path !== bookmark.path && item.name === newName)) {
      showToast("名称已存在");
      return;
    }
  ...
  const handleToggle = (bookmark: DirectoryBookmarkOption) => {
    if (bookmark.mounted) {
      if (unmountBookmark(bookmark.bookmarkId, bookmark.path)) {
        refresh();
        showToast("已取消挂载");
      }
      return;
  ...
  const handleDelete = async (bookmark: DirectoryBookmarkOption) => {
    const confirmed = await Dialog.confirm({
      title: "删除书签",
      message: `确定删除“${bookmark.name}”吗？目录及其中的文件不会被删除。`,
      cancelLabel: "取消",
      confirmLabel: "删除",
    });
    if (!confirmed) return;
  ...
  return (
    ...
    >
      {directoryBookmarks.length > 0 ? (
        ...
        >
          {directoryBookmarks.map((bookmark, index) => (
            <VStack
              key={bookmark.bookmarkId || bookmark.path}
              listRowSeparator={index === 0 ? { visibility: "hidden", edges: "top" } : undefined}
              trailingSwipeActions={{
                allowsFullSwipe: false,
                actions: [
                  <Button title="删除" role="destructive" action={() => handleDelete(bookmark)} />,
                  <Button title="编辑名称" tint="systemOrange" action={() => handleRename(bookmark)} />,
                ],
              }}
              contextMenu={{
                menuItems: (
                  <Group>
                    <Button title="编辑名称" systemImage="pencil" action={() => handleRename(bookmark)} />
                    <Button title="删除书签" systemImage="trash" role="destructive" action={() => handleDelete(bookmark)} />
                  </Group>
                ),
              }}
            >
              <HStack
                ...
                padding={{ vertical: 4 }}
                contentShape="rect"
                onTapGesture={() => handleToggle(bookmark)}
              >
                <Image systemName="folder.fill" frame={{ width: 28, height: 28 }} foregroundStyle={bookmark.mounted ? "systemBlue" : "secondaryLabel"} />
                <VStack alignment="leading" spacing={2} frame={{ maxWidth: "infinity", alignment: "leading" }}>
                  <ScrollView axes="horizontal" scrollIndicator="hidden" frame={{ maxWidth: "infinity" }}>
                    <Text font="body" fixedSize={{ horizontal: true, vertical: false }}>{bookmark.name}</Text>
                  </ScrollView>
                  <Text font="caption2" foregroundStyle="secondaryLabel" lineLimit={1}>{bookmark.path}</Text>
                </VStack>
                <Spacer />
                <Image
                  systemName={bookmark.mounted ? "checkmark.circle.fill" : "circle"}
                  frame={{ width: 22, height: 22 }}
                  foregroundStyle={bookmark.mounted ? "systemBlue" : "tertiaryLabel"}
                />
              </HStack>
            </VStack>
          ))}
        ...
        <Section>
          <VStack alignment="center" spacing={8} padding={{ vertical: 32 }} frame={{ maxWidth: "infinity" }}>
            <Image systemName="folder.badge.questionmark" frame={{ width: 36, height: 36 }} foregroundStyle="tertiaryLabel" />
            <Text font="body" foregroundStyle="secondaryLabel">暂无可用的目录书签</Text>
          </VStack>
        </Section>
      )}
    </List>
...
export function SettingsTabPage({ settings, onSettingsChange, bookmarks, onSwitchTab, onBookmarksChange }: SettingsTabPageProps) {
  // 本地状态，修改后立即更新显示
  ...
  const [showMountedDirectories, setShowMountedDirectories] = useState(false);
  ...
  const handleOpenDualBrowser = () => {
    onSettingsChange({ ...settings, dualModeEnabled: true });
    onSwitchTab?.(0);
  };
    ...
    <NavigationStack>
      <List
        ...
        navigationDestination={{
          ...
          onChanged: setShowMountedDirectories,
          content: <MountedDirectoriesSettingsPage onBookmarksChange={onBookmarksChange} />,
        }}
      >
        {/* ── 核心功能一览 ── */}
          <FeatureRow
            ...
            action={() => setShowMountedDirectories(true)}
            hideTopSeparator
          />
          <FeatureRow
