...
export async function postDownloadAction(record: DownloadSuccess, mode: SaveMode): Promise<string> {
  if (mode === "photos") {
    await saveFilePathToPhotos(record.filePath, record.fileName)
    return "已按默认偏好保存到相册。"
  }

  if (mode === "files") {
    const path = await exportFilePathToFiles(record.filePath, record.fileName)
    return `已按默认偏好导出到文件：${path}`
  }

  const result = await Dialog.actionSheet({
    title: "下载完成",
    message: `《${record.extracted.title || record.fileName}》已下载。接下来你想做什么？`,
    actions: [
      { label: "保存到相册" },
      { label: "导出到文件" },
      { label: "分享文件" },
  ...
  if (result === 2) {
    await ShareSheet.present([record.filePath])
    return "已打开分享面板。"
  }
  return "已加入下载历史。"
}
...
export async function saveResult(download: DownloadSuccess, mode: Exclude<SaveMode, "ask">) {
  if (mode === "photos") {
    await saveFilePathToPhotos(download.filePath, download.fileName)
    return {
      mode,
      message: `已保存到相册：${download.fileName}`,
    }
  }

  const exported = await exportFilePathToFiles(download.filePath, download.fileName)
  return {
    mode,
    message: `已导出文件：${exported}`,
  }
}
